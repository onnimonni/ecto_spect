defmodule EctoVerify.ConfigTest do
  use ExUnit.Case, async: true

  alias EctoVerify.Config
  ...
  describe "new/1" do
    ...
    test "accepts explicit rule list" do
      config = Config.new(repos: [MyApp.Repo], rules: [EctoVerify.Rules.SequentialScan])
      assert config.rules == [EctoVerify.Rules.SequentialScan]
    end
    ...
    test "respects ignore_rules" do
      ...
      refute EctoVerify.Rules.MissingLimit in config.rules
      assert EctoVerify.Rules.SequentialScan in config.rules
    end
