defmodule EctoVerify.FormatterTest do
  use ExUnit.Case, async: true

  alias EctoVerify.Formatter

  defp violation(opts \\ []) do
    %EctoVerify.Violation{
      rule: EctoVerify.Rules.SequentialScan,
  ...
  describe "summary/1" do
    test "formats single error violation" do
      ...
      assert String.starts_with?(result, "EctoVerify: ")
    ...
    test "includes rule short name (last module segment)" do
      ...
      assert String.contains?(result, "SequentialScan")
      refute String.contains?(result, "EctoVerify.Rules.SequentialScan")
    end
  ...
  describe "print/2 with :silent output" do
    test "returns :ok without printing" do
      config = %EctoVerify.Config{output: :silent}
  ...
  describe "print/2 with empty list" do
    test "returns :ok immediately" do
      assert :ok = Formatter.print([], %EctoVerify.Config{})
    end
  end

  describe "print_schema/2" do
    test "returns :ok for empty list" do
      assert :ok = Formatter.print_schema([], %EctoVerify.Config{output: :plain})
    end
  end
end
