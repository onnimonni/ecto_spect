defmodule EctoSpect.Rules.UnusedIndexesTest do
  use ExUnit.Case, async: true

  alias EctoSpect.Rules.UnusedIndexes
  ...
  describe "setup_suite/1 and Application env" do
    test "snapshot_key is stored in Application env after setup_suite" do
      ...
      Application.delete_env(:ecto_spect, :ecto_spect_index_scan_snapshot)
      assert Application.get_env(:ecto_spect, :ecto_spect_index_scan_snapshot) == nil
    end
  end
  ...
  defp build_test_violation([index_name, table_name, _scans, _pk, is_unique]) do
    ...
    %EctoSpect.Violation{
      rule: EctoSpect.Rules.UnusedIndexes,
      severity: :warning,
      message:
