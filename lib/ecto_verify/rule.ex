defmodule EctoVerify.Rule do
  @moduledoc """
  Behaviour that every EctoVerify rule implements.
  ...
  @callback check(plan_nodes(), entry(), thresholds :: map()) :: [EctoVerify.Violation.t()]

  @doc "Check all queries captured in a test for cross-query violations (e.g. N+1)."
  @callback check_group(entries :: [entry()], thresholds :: map()) :: [EctoVerify.Violation.t()]

  @doc "Check schema-level issues using a dedicated DB connection. Called once per suite."
  @callback check_schema(conn :: pid(), thresholds :: map()) :: [EctoVerify.Violation.t()]
  ...
  """
  @callback check_suite_end(conn :: pid(), thresholds :: map()) :: [EctoVerify.Violation.t()]
