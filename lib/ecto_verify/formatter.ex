defmodule EctoVerify.Formatter do
  @moduledoc """
  Formats `EctoVerify.Violation` structs as Credo-style terminal output.
  ...
  @spec print([EctoVerify.Violation.t()], keyword()) :: :ok
  def print(violations, config \\ %EctoVerify.Config{})

  def print([], _config), do: :ok

  def print(violations, %EctoVerify.Config{output: :silent}), do: violations && :ok

  def print(violations, config) do
    use_color = match?(%EctoVerify.Config{output: :ansi}, config) and IO.ANSI.enabled?()
    count = length(violations)
    label = if count == 1, do: "violation", else: "violations"

    header = "\nEctoVerify found #{count} #{label}:\n"
  ...
  @spec print_schema([EctoVerify.Violation.t()], map()) :: :ok
  def print_schema([], _config), do: :ok

  def print_schema(violations, config) do
    use_color = match?(%EctoVerify.Config{output: :ansi}, config) and IO.ANSI.enabled?()
    header = "\nEctoVerify schema warnings:\n"
  ...
  @spec summary([EctoVerify.Violation.t()]) :: String.t()
  ...
  defp format_caller(stacktrace) do
    # Find the first frame that is not from Ecto/EctoVerify internals
    stacktrace
    |> Enum.find(fn
      {mod, _fun, _arity, info} ->
        ...
        not String.starts_with?(mod_str, "Ecto") and
          not String.starts_with?(mod_str, "EctoVerify") and
