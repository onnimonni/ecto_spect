defmodule EctoSpect.Rules.UnusedIndexes do
  ...
  @snapshot_key :ecto_spect_index_scan_snapshot
  ...
  def setup_suite(conn) do
    case Postgrex.query(conn, @stats_sql, []) do
      {:ok, %{rows: rows}} ->
        ...
        Application.put_env(:ecto_spect, @snapshot_key, snapshot)

      {:error, reason} ->
        require Logger
        Logger.debug("[EctoSpect] UnusedIndexes snapshot failed: #{inspect(reason)}")
    end
  ...
  def check_suite_end(conn, _thresholds) do
    snapshot = Application.get_env(:ecto_spect, @snapshot_key, %{})
    ...
    case Postgrex.query(conn, @stats_sql, []) do
      ...
      {:error, reason} ->
        require Logger
        Logger.debug("[EctoSpect] UnusedIndexes suite-end check failed: #{inspect(reason)}")
        []
    end
  end
  ...
  defp build_violation([index_name, table_name, _scans, _pk, is_unique]) do
    ...
    %EctoSpect.Violation{
      ...
      1. Missing test coverage — maybe you forgot to add tests which use this index?
         ...
         EctoSpect will confirm the index is used once the query runs through it.
