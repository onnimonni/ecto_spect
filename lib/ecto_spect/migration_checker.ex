defmodule EctoSpect.MigrationChecker do
  @moduledoc """
  ...
  Migration checks run once per test suite (in `EctoSpect.setup/1`) and
  ...
  """
  @spec check(EctoSpect.Config.t()) :: [EctoSpect.Violation.t()]
  def check(%EctoSpect.Config{repos: repos, rules: rules}) do
  ...
  defp migration_files_for(repo) do
    ...
    case File.ls(path) do
      ...
      {:error, reason} ->
        Logger.debug("[EctoSpect] MigrationChecker: cannot list #{path}: #{inspect(reason)}")
  ...
  defp check_file(path, rules) do
    ...
    ast =
      try do
        ...
        e ->
          Logger.debug("[EctoSpect] MigrationChecker: failed to parse #{path}: #{inspect(e)}")
    ...
    if ast do
      Enum.flat_map(rules, fn rule ->
        try do
          ...
          e ->
            Logger.debug("[EctoSpect] MigrationChecker: #{rule} failed on #{path}: #{inspect(e)}")
