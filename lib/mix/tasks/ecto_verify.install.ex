defmodule Mix.Tasks.EctoVerify.Install do
  @moduledoc """
  Installs EctoVerify into the current Phoenix/Ecto project.
  ...
  1. Inserts `EctoVerify.setup/1` before `ExUnit.start()` in `test/test_helper.exs`
  2. Adds `use EctoVerify.Case` inside the `quote do` block of your `DataCase`
     (detected automatically from `test/support/data_case.ex`)
  """
  @shortdoc "Installs EctoVerify into a Phoenix/Ecto project"
  ...
  def info(_argv, _composing_task) do
    %Igniter.Mix.Task.Info{
      group: :ecto_verify,
      example: "mix ecto_verify.install",
      schema: [repo: :string],
      aliases: [r: :repo]
    }
  end
  ...
  defp patch_test_helper(igniter, repo) do
    Igniter.update_elixir_file(igniter, "test/test_helper.exs", fn zipper ->
      setup_call = "EctoVerify.setup(repos: [#{inspect(repo)}])\n"
  ...
  defp patch_data_case(igniter, repo) do
    ...
    if File.exists?(path) do
      Igniter.update_elixir_file(igniter, path, fn zipper ->
        use_line = "use EctoVerify.Case, repo: #{inspect(repo)}"
          ...
          quote_zipper ->
            ...
            already_installed =
              find_node(quote_zipper, fn node ->
                match?(
                  {:use, _, [{:__aliases__, _, [:EctoVerify, :Case]} | _]},
                  node
                )
              end)
      ...
      Igniter.add_warning(
        ...
        "Could not find test/support/data_case.ex. " <>
          "Add `use EctoVerify.Case, repo: #{inspect(repo)}` manually inside your DataCase `using` block."
      )
    end
  end
